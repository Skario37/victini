module.exports = (client) => {
    ...
    DONNEZ JAMAIS LEVEL 10 A QUELQU'UN D'AUTRE QUE LE PROPRIETAIRE !!!
    Par défaut on peut utiliser les très dangereuses commandes `eval` et `exec` !
    */
    client.permlevel = message => {
      let permlvl = 0;
  
      const permOrder = client.permlevelSettings.slice(0).sort((p, c) => p.level < c.level ? 1 : -1);
    ...
    // Génère un cache des permissions du client
    client.levelCache = {};
    for (let i = 0; i < client.permlevelSettings.length; i++) {
      const thisLevel = client.permlevelSettings[i];
